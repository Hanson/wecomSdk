...
func TestRealTokenAndAgent(t *testing.T) {
	...
	if corpID == "" || secret == "" || agentStr == "" {
		t.Skip("missing WECOM_CORP_ID / WECOM_APP_SECRET / WECOM_AGENT_ID")
	}
	...
	if err != nil || tok == "" {
		if e, ok := err.(*Error); ok && e.Code == 60020 {
			t.Skip("可信IP未配置，跳过真实接口测试")
		}
	...
	if a.AgentID != agentID {
		t.Fatalf("agentid mismatch: %d", a.AgentID)
	}
}
