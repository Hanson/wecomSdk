...
func TestRealSendTextMessage(t *testing.T) {
	...
	if corpID == "" || secret == "" || agentStr == "" || toUser == "" {
		t.Skip("missing env: WECOM_CORP_ID/WECOM_APP_SECRET/WECOM_AGENT_ID/WECOM_TO_USER")
	}
	...
	if r.InvalidUser != "" {
		t.Skip("invaliduser: " + r.InvalidUser)
	}
	if r.MsgID == "" {
		t.Fatalf("empty msgid")
	}
}
