...
func TestRealExternalContact(t *testing.T) {
    ...
    if userID == "" { userID = os.Getenv("WECOM_TO_USER") }
    if corpID == "" || secret == "" || userID == "" { t.Skip("missing env: WECOM_CORP_ID/WECOM_APP_SECRET/WECOM_USER_ID") }
    ...
    if len(listOut.ExternalUserID) == 0 { t.Skip("no external contacts bound to user: " + userID) }
    ...
    if getOut.ExternalContact.ExternalUserID == "" { t.Fatalf("empty external user id") }
