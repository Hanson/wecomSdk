...
func TestGetAccessTokenCaching(t *testing.T) {
    ...
    tok1, err := c.GetAccessToken(ctx); if err != nil { t.Fatalf("get token: %v", err) }
    tok2, err := c.GetAccessToken(ctx); if err != nil { t.Fatalf("get token 2: %v", err) }
    if tok1 != "TOKEN1" || tok2 != "TOKEN1" { t.Fatalf("unexpected tokens: %s %s", tok1, tok2) }
    if atomic.LoadInt32(&cnt) != 1 { t.Fatalf("gettoken called %d times", cnt) }
}
...
func TestSendTextMessage(t *testing.T) {
    ...
    ctx := context.Background()
    r, err := c.SendTextMessage(ctx, msg); if err != nil { t.Fatalf("send: %v", err) }
    if r.MsgID != "123" { t.Fatalf("unexpected msgid: %s", r.MsgID) }
}
...
func TestTokenAutoRefresh(t *testing.T) {
    ...
    tok2, err := c.GetAccessToken(ctx); if err != nil { t.Fatalf("get token 2: %v", err) }
    if tok1 != "T1" || tok2 != "T1" { t.Fatalf("unexpected tokens: %s %s", tok1, tok2) }
    if atomic.LoadInt32(&cnt) < 2 { t.Fatalf("expected refresh, calls: %d", cnt) }
}
